...
<section class="content">
    ...
    ">
        ...
        <div class="absolute inset-0 bg-cover bg-center shadow-md" style="background-image: url('{{ asset('storage/' . $foto->foto_trafo) }}');">
...
<a href="{{url('/trafo/excel/export')}}"><button class="mt-5 mx-10 shadow-md text-white bg-blue-700 p-2 rounded-lg duration-200">Export</button></a>
        ...
        <table class="w-full">
            <thead>
                <tr class="text-sm text-left bg-blue-700 text-white">
                    ...
                    <th class="p-3 tracking-wide whitespace-nowrap">Lokasi GI</th>
                    <th class="p-3 tracking-wide whitespace-nowrap">Nama Bay</th>
                    <th class="p-3 tracking-wide whitespace-nowrap">Merk Trafo</th>
                    <th class="p-3 tracking-wide whitespace-nowrap">Tahun Operasi</th>
                    <th class="p-3 tracking-wide whitespace-nowrap">Ratio Trafo</th>
                    <th class="p-3 tracking-wide whitespace-nowrap">Rata Rata Pembebanan</th>
                </tr>
            </thead>
            @php
                $counter = 1;
            ...
            <tr class="text-sm text-left bg-slate-50 border-b-2 border-gray-200">
                ...
                <td class="p-3 whitespace-nowrap">{{ $trafo->lokasi_gi }}</td>
                <td class="p-3 whitespace-nowrap">{{ $trafo->nama_bay }}</td>
                <td class="p-3 whitespace-nowrap">{{ $trafo->merk_trafo }}</td>
                <td class="p-3 whitespace-nowrap">{{ $trafo->tahun_operasi }}</td>
                <td class="p-3 whitespace-nowrap">{{ $trafo->ratio_trafo }}</td>
                <td class="p-3 whitespace-nowrap">{{ $trafo->rata_rata_pembebanan }}</td>
            </tr>
            @endforeach
            @else
            <tr>
